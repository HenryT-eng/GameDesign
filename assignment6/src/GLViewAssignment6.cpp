...
GLViewAssignment6* GLViewAssignment6::New( const std::vector< std::string >& args )
{
   GLViewAssignment6* glv = new GLViewAssignment6( args );
...
GLViewAssignment6::GLViewAssignment6( const std::vector< std::string >& args ) : GLView( args )
{
   //Initialize any member variables that need to be used inside of LoadMap() here.
   //Note: At this point, the Managers are not yet initialized. The Engine initialization
   //occurs immediately after this method returns (see GLViewAssignment6::New() for
   ...
   //GLViewAssignment6::onCreate() is invoked after this module's LoadMap() is completed.
}


void GLViewAssignment6::onCreate()
{
   //GLViewAssignment6::onCreate() is invoked after this module's LoadMap() is completed.
...
GLViewAssignment6::~GLViewAssignment6()
{
   //Implicitly calls GLView::~GLView()
}


void GLViewAssignment6::updateWorld()
{
   GLView::updateWorld(); //Just call the parent's update world first.
                          //If you want to add additional functionality, do it after
                          //this call.
   GLViewAssignment6::cube->setPosition(Vector(MoveX, MoveY, MoveZ));
   //NetMsgCreateWO msg;
   //msg.xPos = MoveX;
   //msg.yPos = MoveY;
   //msg.zPos = MoveZ;
...
void GLViewAssignment6::onResizeWindow( GLsizei width, GLsizei height )
{
   GLView::onResizeWindow( width, height ); //call parent's resize method.
}


void GLViewAssignment6::onMouseDown( const SDL_MouseButtonEvent& e )
{
   GLView::onMouseDown( e );
}


void GLViewAssignment6::onMouseUp( const SDL_MouseButtonEvent& e )
{
   GLView::onMouseUp( e );
}


void GLViewAssignment6::onMouseMove( const SDL_MouseMotionEvent& e )
{
   GLView::onMouseMove( e );
}


void GLViewAssignment6::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   if( key.keysym.sym == SDLK_1 )
   {
       ...
       msg.zPos = MoveZ;
       client->sendNetMsgSynchronousTCP(msg);
       //NetMsg* reply = client->sendNetMsgBlockForReplyTCP(msg);
   }
}


void GLViewAssignment6::onKeyUp( const SDL_KeyboardEvent& key )
{
   GLView::onKeyUp( key );
}


void Aftr::GLViewAssignment6::loadMap()
{
   ...
   GLViewAssignment6::cube = WO::New(shinyRedPlasticCube, Vector(1, 1, 1), MESH_SHADING_TYPE::mstFLAT);
   GLViewAssignment6::cube->renderOrderType = RENDER_ORDER_TYPE::roOPAQUE;
   GLViewAssignment6::cube->setLabel("cube");
   ...
   createAssignment6WayPoints();
}


void GLViewAssignment6::createAssignment6WayPoints()
{
